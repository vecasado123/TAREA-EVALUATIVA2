...
CREATE DATABASE `dwess` DEFAULT CHARACTER SET utf8 COLLATE utf8_spanish_ci;
USE `dwess`;

-- Creamos las tablas
CREATE TABLE `dwess`.`tienda` (
`cod` INT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
`nombre` VARCHAR( 100 ) NOT NULL ,
`tlf` VARCHAR( 13 ) NULL
) ENGINE = INNODB;

CREATE TABLE `dwess`.`producto` (
...
CREATE TABLE `dwess`.`familia` (
`cod` VARCHAR( 6 ) NOT NULL ,
`nombre` VARCHAR( 200 ) NOT NULL ,
PRIMARY KEY ( `cod` )
) ENGINE = INNODB;

CREATE TABLE `dwess`.`stock` (
